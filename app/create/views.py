...
def create_experiment():
    ...
    user_groups = user_in_group.query.filter_by(iduser=session["_user_id"]).all()

    if user_groups:
        user_groups = [ug.idgroup for ug in user_groups]

        valid_groups = research_group.query.filter(
            research_group.id.in_(user_groups)
        ).all()
    ...
    if request.method == "POST" and form.validate():
        ...
        new_exp = experiment(
            ...
            status="Hidden",
            randomization="Off",
            single_sentence_instruction=request.form["single_sentence_instruction"],
            short_instruction=request.form["short_instruction"],
            creator_name=request.form["creator_name"],
            is_archived="False",
            creation_time=the_time,
            stimulus_size="7",
    ...
    return render_template(
        "create_experiment.html", form=form, valid_groups=valid_groups
    )
...
def experiment_bgquestions():
            ...
            for x in range(len(list)):
                ...
                if x == 0:
                    ...
                    add_bgq_option = background_question_option(
                        background_question_idbackground_question=add_bgquestion.idbackground_question,
                        option=list[x],
                    )
    ...
    return render_template(
        "create_experiment_bgquestions.html", form=form, exp_id=exp_id
    )
    ...
    if request.method == "POST":
        ...
        for a in range(len(str_list)):
            ...
            add_question = question(
                experiment_idexperiment=exp_id,
                question=list[0],
                left=list[1],
                right=list[2],
            )
    ...
    if request.method == "POST" and form.validate():
        ...
        if form.type.data == "text":
            ...
            for file in request.files.getlist("file"):
                ...
                new_page = page(
                    experiment_idexperiment=exp_id, type=form.type.data, media=db_path
                )
